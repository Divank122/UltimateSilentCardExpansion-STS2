...
public class DissolveForge : SilentCardModel, ILocalizationProvider
{
    ...
    protected override async Task OnPlay(PlayerChoiceContext choiceContext, CardPlay cardPlay)
    {
        ...
        {
            await PowerCmd.Apply<PoisonPower>(enemy, DynamicVars["PoisonPower"].IntValue, Owner.Creature, this);
        }

        if (IsUpgraded)
        {
            await PowerCmd.Apply<DissolveForgePowerPlus>(Owner.Creature, 1, Owner.Creature, this);
        }
        else
        {
            await PowerCmd.Apply<DissolveForgePower>(Owner.Creature, 1, Owner.Creature, this);
        }
    }
